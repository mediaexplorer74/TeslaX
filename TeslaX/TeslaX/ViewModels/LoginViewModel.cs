...
namespace TeslaX.ViewModels
{
    ...
    {
        private TeslaAuthHelper teslaAuth = new TeslaAuthHelper("TeslaX/0.0");
        ...
        private async Task GetSiteId()
        {
            ...
            else
            {
                ...
                Debug.WriteLine("Powerwall site not found");

                // TEMP : RnD
                string siteName = "Powerwall site not found";
                long id = 1234; // fake id (RnD only)

                availableSites.Add(id.ToString(), siteName);
                Settings.AvailableSites = availableSites;
                await Settings.SavePropertiesAsync(); // RnD: save settings
            }
